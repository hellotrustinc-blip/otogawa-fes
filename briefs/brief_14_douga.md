...
**2. 動画の著作権について**（2026-07-24夜・発注元指示で譲渡型に改訂）
投稿された動画の著作権（著作財産権）は、投稿をもって大戸川祭礼保存会に譲渡されるものとします。保存会は、祭礼の記録・保存および保存会の活動（記録集・広報・サイト掲載等）のために、動画を無償で利用（保管・複製・編集・上映・掲載）できます。投稿者は著作者人格権を行使しないものとし、投稿後の動画について削除・返却の申し立てはできないものとします。
...
## 追補7（発注元の実機テスト指摘: 手順書型UI・ファイル選択ボタン・準備中表示・警告位置／承認済み・完遂せよ）

発注元が実機スマホでテストした際の指摘4点。douga.html / douga.js のみ変更・GAS変更禁止。

### 1. ファイル選択の自前ボタン化
- ブラウザ標準の「ファイル選択」ボタンは文言変更不可のため、`input[type=file]` を視覚的に隠し（`display:none`等・アクセシビリティ維持）、**自前のボタン「📁 ファイルを選ぶ」**（朱色・大きめ・タップしやすい）で `input.click()` を発火する方式に変える
- 選択後はボタンの近くに「選択中: ファイル名（複数時は「◯本の動画を選択中」）」を表示。未選択時は「まだ選ばれていません」

### 2. 手順書型UI（番号ステップ形式）
- 投稿フォーム全体を「やることが上から順に分かる」構成に再編する:
  - **手順1. お名前を入力（ニックネーム可・省略可）** → お名前欄
  - **手順2. 「ファイルを選ぶ」を押して動画を選ぶ（複数可）** → 自前ボタン＋選択状態表示。近くに補足「ボタンを押すとスマホの動画一覧が開きます」
  - **手順3. 下の規約を確認** → 規約スクロール枠
  - **手順4. 「同意してアップロード」を押す** → 同意ボタン（既存文言のまま）＋補足「ボタンを押すと、上記への同意のうえ投稿されます」
- 手順番号は丸数字風のバッジ（朱色）で視認性を上げる。既存の注意書き（祭礼と無関係な動画は削除します等）は手順の上か下に自然に残す

### 3. 「アップロード準備中…」表示
- 同意ボタン押下の瞬間に、ボタン無効化と同時に**「アップロード準備中…（数秒かかります）」**をボタン直下または進捗エリアに即時表示する（受付処理の往復で数秒無反応に見える問題への対策）。最初のチャンク送信が始まったら通常の進捗表示に切り替える

### 4. 「閉じないで」警告の位置を下部へ
- 注意帯「⚠ アップロード中です。終わるまで画面を閉じたり…」を、**進捗表示（ファイルごとの進捗行）の直下**に移動する（アップ中はページ下部を見ているため上部では見えない・発注元の実機観察）
- 完了時の「✅ 投稿が完了しました。動画は保存会にお届けしました。画面を閉じて大丈夫です」も同じ下部位置に出す

### 合格条件（すべて exit 0）
1. `node --check` 全 `.js`/`.mjs`・既存テスト2本全PASS
2. テストで機械確認: 「ファイルを選ぶ」ボタン文言／手順1〜4の見出し文言／「アップロード準備中」文言が douga.html または douga.js に存在すること
3. テストで機械確認: DOM順で注意帯（upload-alert）が進捗リスト（upload-list）より後にあること（HTMLソースの出現順検査で可）
4. mockモード: 手順UI＋選択中表示＋50%ダミー進捗＋下部注意帯が表示されること（レンダリング実測はClaude側）

---

## 追補8（発注元指示: お問い合わせフォームを投稿ページ最下部にも設置／承認済み・完遂せよ）

index.html 最下部にある既存のお問い合わせフォーム（フォーム入力→GAS直接送信→成功パネル・自動返信案内つき）と**同じ機能・同じ文言**のセクションを、douga.html の**最下部（投稿パネルの下）**にも設置する。GASファイル変更禁止（同じ /exec を使う＝再デプロイ不要）。

### 実装要件
1. **index.html の実装を先に読み**、お問い合わせセクション（フォーム・hidden iframe送信・onContactSubmit相当のJS・送信完了パネル）を douga.html へ移植する。送信先は douga.html 冒頭の `GAS_URL` 定数を再利用（URL直書きの重複を避ける。フォームのaction属性はJSで `GAS_URL` を設定してよい）
2. **文言は index.html と完全一致**を保つこと。特に送信完了パネルの「返信が来ない場合＝届いていない/迷惑メールフォルダの可能性」案内（2026-07-02に発注元が指定した文言）は一字一句変えない
3. id/クラスは douga.html 既存のもの（upload-form 等）と衝突させない（cf- 接頭辞等で分離）
4. 見た目は douga.html のトーン（パネル・朱色ボタン）に合わせる。セクション見出し「お問い合わせ」
5. 配置=投稿パネル（および注意帯・進捗）より後・ページ最下部
6. mockモード（?mock=1）でもフォームが表示されること

### 合格条件（すべて exit 0）
1. `node --check` 全 `.js`/`.mjs`・既存テスト2本全PASS
2. テストで機械確認: douga.html に「お問い合わせ」セクション・お名前/メール/内容の入力欄・迷惑メール案内文言（index.htmlと同一文字列）が存在すること／DOM順で投稿パネルより後にあること
3. テストで機械確認: index.html のお問い合わせ文言と douga.html の同文言が一致すること（両ファイルから同一部分文字列を抽出して比較）
4. レンダリング実測はClaude側

---

## 追補9（本番利用者障害の対応: 写真（画像）も投稿可能に／承認済み・完遂せよ）

**実障害**: 発注元が実機で写真を投稿しようとしたところ、ファイル選択の「決定」が効かない。原因=`accept="video/*"`と受け皿のMIME検査が動画専用のため、画像が選択対象外になっている。**写真対応に拡張する**。

### 1. クライアント（douga.html / douga.js）
- `input[type=file]` を `accept="video/*,image/*"` に変更
- 文言更新: ヒーロー「祭礼の動画を投稿できます」→「祭礼の動画・写真を投稿できます」／パネル説明「投稿できるのは動画のみ、1本4GBまで」→「投稿できるのは動画・写真、1ファイル4GBまで」／手順2「動画を選ぶ」→「動画・写真を選ぶ」／未選択ガード「動画を選んでください」→「動画・写真を選んでください」／選択中表示「◯本の動画を選択中」→「◯件を選択中」／完了「動画は保存会にお届けしました」→「保存会にお届けしました」
- 規約枠の文言更新（**必要最小限の機械的拡張のみ・構造や他の条文は変えない**）: 見出し「動画の投稿にあたって」→「動画・写真の投稿にあたって」／第1条「動画のみ投稿できます」まわりの「動画」→「動画・写真」／第2条「投稿された動画の著作権」→「投稿された動画・写真の著作権」等、「動画」単語を「動画・写真」に置換（第2条の譲渡・削除申立不可・人格権不行使の構造は一切変えない）
- クライアント側の軽い検査があれば `video/` 始まり限定を `video/` または `image/` 始まりに

### 2. 受け皿（お問い合わせ受け皿.gs — 変更許可・今回のみ）
- `initUpload` のMIME検査を「`video/` または `image/` で始まる」に拡張。エラーメッセージも「動画・写真のファイルのみ投稿できます」に
- 他のアクション・お問い合わせ処理・メール文言・ランキング系（存在する場合）は一切変更しない

### 3. テスト更新
- `tmp/test_gas_router.mjs`: `image/jpeg` の initUpload 正常系assertを追加・`text/plain` は引き続き拒否
- `tmp/test_upload_client.mjs`: accept属性に image/* が含まれること・新文言の存在を機械確認

### 合格条件（すべて exit 0）
1. `node --check` 全 `.js`/`.mjs`
2. `node tmp/test_gas_router.mjs`（image正常系・text拒否・お問い合わせ完全温存assert込み全PASS）
3. `node tmp/test_upload_client.mjs`（accept拡張+新文言assert込み全PASS）
4. レンダリング実測・本番E2E（画像アップロード）はClaude側
