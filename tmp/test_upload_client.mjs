...
assert.match(html, /accept="video\/\*,image\/\*"/);
assert.match(html, /祭礼の動画・写真を投稿できます/);
assert.match(html, /投稿できるのは動画・写真、1ファイル4GBまで/);
assert.match(html, /手順2\. 「ファイルを選ぶ」を押して動画・写真を選ぶ/);
assert.match(html, /動画・写真の投稿にあたって（個人情報の取り扱い・著作権）/);
assert.match(html, /1\. 投稿できる動画・写真/);
assert.match(html, /2\. 動画・写真の著作権について/);
...
assert.match(source, /投稿が完了しました。保存会にお届けしました。画面を閉じて大丈夫です/);
assert.match(source, /files\.length\s*===\s*0/);
assert.match(source, /動画・写真を選んでください/);
assert.match(source, /選択中: \$\{selected\.length\}件を選択中/);
assert.match(source, /startsWith\('video\/'\).*startsWith\('image\/'\)/s);
